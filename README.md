...
AI에게 자연어로 요청해서 코드를 만드는 개발 방식입니다.
코드 문법보다 **무엇을 만들지**에 집중합니다.
...
Amazon AWS가 만든 에이전트형 AI IDE입니다. VS Code와 Claude를 별도로 연동하는 대신, AI가 처음부터 IDE 안에 통합되어 있습니다.

**다운로드:** https://kiro.dev/downloads/

Windows, macOS, Linux 모두 지원하며 현재 Preview 기간 중 무료로 사용할 수 있습니다.

### 핵심 기능

**스펙 기반 개발 (Spec-Driven Development)**
자연어 요청을 받으면 바로 코드를 생성하는 대신, 요구사항 정의 → 시스템 설계 → 태스크 분리 → 코드 생성 순서로 진행합니다. 복잡한 프로젝트에서 방향성을 잃지 않도록 도와줍니다.

**에이전트 훅 (Agent Hooks)**
파일 저장, 커밋 등의 이벤트에 AI를 자동으로 트리거합니다. 예를 들어 파일 저장 시 자동 린팅이나 보안 스캔을 실행할 수 있습니다.

**스티어링 (Steering)**
팀의 코딩 컨벤션을 마크다운 파일로 정의하면 Kiro가 해당 규칙에 맞게 코드를 생성합니다.

**VS Code 호환**
기존 VS Code의 설정, 테마, 플러그인을 그대로 가져올 수 있습니다.
...
Adversarial Attacks 강의에서 다룬 개념을 바이브 코딩으로 직접 실험해봅니다.
MNIST 대신 sklearn으로 tabular 데이터를 생성해서 실습하므로 노트북에서도 가볍게 실행됩니다.
아래 프롬프트를 Claude 또는 Kiro에 그대로 입력해서 코드를 생성해보세요.
